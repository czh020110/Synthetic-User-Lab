...
1. 开始任何与当前项目相关的实现、修改、验证任务前，先判断是否需要调用本 skill 更新任务板。
2. 不要重排已有 `T-xxx` 编号。
3. 如果任务板中已有未完成任务，不要覆盖原条目；只有新增需求时才追加。
4. 完成任务时必须填写 `--verification`；有 commit 时再填写 `--commit`。
5. 若只是确认脚本效果，先用 `--dry-run --json`。
6. 若用户明确要求批量或特殊格式调整，再考虑直接编辑任务板或脚本。
